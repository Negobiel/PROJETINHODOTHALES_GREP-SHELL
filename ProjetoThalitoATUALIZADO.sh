...
VEGA=$(echo $NOME | grep -E '^[A-Z|a-z]{1,}$');
    if [[ $? -eq 0 ]]; then
        sleep 1
        echo "Nome Valído"
        sleep 2
...
while [ resultado==1 ]; do
    ...
    DAT=$(echo $DNAS | grep -E '(0[0-9]|[1,2][0-9]|3[0,1])[-/.](0[1-9]|1[1,2])[-/.][0-9]{4}');
    if [[ $? -eq 0 ]]; then
...
while [ resultado==1 ]; do
    ...
    PI=$(echo $IP | grep -E '^([0-9]|[0-9][0-9]|1[0-9][0-9]|2[0-5][0-5])\.([0-9]|[0-9][0-9]|1[0-9][0-9]|2[0-5][0-5])\.([0-9]|[0-9][0-9]|1[0-9][0-9]|2[0-5][0-5])\.([0-9]|[0-9][0-9]|1[0-9][0-9]|2[0-5][0-5])');
    if [[ $? -eq 0 ]]; then
